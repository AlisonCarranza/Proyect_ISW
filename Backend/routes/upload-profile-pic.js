...
router.post('/api/upload-profile-pic', dev.single('file'), async(req, res) => {
    try {
        ...
        const perfilPath = path.join(__dirname, "C:/Users/Administrator/Documents/Proyect_ISW/Backend/upload", User.picPerfil);          
            email = User.email; 
            if(await user.updateOne({email},{$set:{picPerfil:dir}})){
                defaultPath=path.join(__dirname, "C:/Users/Administrator/Documents/Proyect_ISW/Backend/upload/Default.png")
